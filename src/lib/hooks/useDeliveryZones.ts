...
export function useCreateDeliveryZoneMutation() {
    ...
    return useMutation({
        mutationFn: ({ restaurantId, zoneName, flatRate, freeThreshold }: { restaurantId: string, zoneName: string, flatRate: number, freeThreshold?: number }) => 
            createDeliveryZone(restaurantId, zoneName, flatRate, freeThreshold),
...
export function useUpdateDeliveryZoneMutation() {
    const queryClient = useQueryClient();
    return useMutation({
        mutationFn: ({ id, zoneName, flatRate, freeThreshold, isActive }: { id: string, zoneName: string, flatRate: number, freeThreshold?: number, isActive: boolean }) => 
            updateDeliveryZone(id, zoneName, flatRate, freeThreshold, isActive),
        onSuccess: () => {
